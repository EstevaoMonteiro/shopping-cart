 ...
 */
  ...
  */
  // Constantes para o DOM.
  const loading = document.getElementsByClassName('loading')[0];
  const cartItems = document.getElementsByClassName('cart__items')[0];
  const cart = document.querySelector('.cart');
  const totalValue = document.createElement('h1');
  ...
  */
  ...
  const totalPrice = () => {
    const value = [];
    for (let i = 0; i <= cartItems.children.length - 1; i += 1) {
      value.push(Number(cartItems.children[i].getAttribute('price')));
    }
    const add = value.reduce((a, b) => a + b, 0);
    totalValue.innerText = `R$ ${add}`;
  };
